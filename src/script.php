...
class mod_cloudpaymentsInstallerScript
{
    /**
	...
	public function preflight($type, $parent)
	{
		...
		if (!class_exists('Joomla\CMS\Version')) {
			JFactory::getApplication()->enqueueMessage(JText::sprintf('MOD_CLOUDPAYMENTS_ERROR_COMPATIBLE_JOOMLA',
		...
		if (!(version_compare(PHP_VERSION, $this->minimumPhp) >= 0)) {
			$app->enqueueMessage(Text::sprintf('MOD_CLOUDPAYMENTS_ERROR_COMPATIBLE_PHP',
		...
		if (!$jversion->isCompatible($this->minimumJoomla)) {
			$app->enqueueMessage(Text::sprintf('MOD_CLOUDPAYMENTS_ERROR_COMPATIBLE_JOOMLA',
				$this->minimumJoomla), 'error');
